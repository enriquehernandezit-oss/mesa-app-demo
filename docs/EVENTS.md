...
`packages/db/src/schema.ts`, table `events`:
...
| `coverImageId`         | a full R2 URL, optional — falls back to the restaurant's own cover when null                                                                                                                                                                                                                                                             |
...
shared event link can all point at one. Taking an event off the calendar —
it got cancelled, or it was a mistake
